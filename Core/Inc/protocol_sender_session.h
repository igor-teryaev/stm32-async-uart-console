...
bool protocol_sender_session_init(
...
bool protocol_sender_session_reset(
    ProtocolSenderSession *session,
    uint16_t initial_sequence
);
// кодує і зберігає новий COMMAND
bool protocol_sender_session_start_command(
    ProtocolSenderSession *session,
...
bool protocol_sender_session_get_transmission(
    const ProtocolSenderSession *session,
    const uint8_t **data,
    size_t *length,
    uint16_t *sequence
);
// transport прийняв кадр, спроба витрачена
bool protocol_sender_session_mark_accepted(
    ProtocolSenderSession *session
);
// останній байт переданий, запускається timeout
bool protocol_sender_session_mark_transmitted(
    ProtocolSenderSession *session,
    uint32_t now_ms
);
// transport повідомив про помилку передачі
bool protocol_sender_session_mark_transmit_error(
    ProtocolSenderSession *session
);
// перевіряє timeout без блокування
void protocol_sender_session_poll(
    ProtocolSenderSession *session,
    uint32_t now_ms
);
// поновлює timeout для правильного sequence
bool protocol_sender_session_handle_in_progress(
    ProtocolSenderSession *session,
    uint16_t sequence,
    uint32_t now_ms
);
...
bool protocol_sender_session_release_result(
    ProtocolSenderSession *session
);
#endif
